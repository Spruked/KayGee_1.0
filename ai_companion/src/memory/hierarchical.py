...
class HierarchicalMemory:
    ...
    def nightly_consolidation(self):
        ...
        if len(old_cases) >= 20:  # Lower threshold for practicality
            vectors = np.array([json.loads(c['features_vec']) for c in old_cases])
    ...
    def compute_prototype(self, cluster_cases: List[Dict]) -> Dict:
        vectors = np.array([json.loads(c['features_vec']) for c in cluster_cases])
        centroid_vec = vectors.mean(axis=0).tolist()
        
        actions = [c['action'] for c in cluster_cases if c['action']]
        most_common_action = Counter(actions).most_common(1)[0][0] if actions else None
        
        scores = [c['ethical_score'] for c in cluster_cases if c['ethical_score'] is not None]
